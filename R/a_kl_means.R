...
#'
#'@export
kl_means <- function(paths, clust_out, max_iter = 1, dist_measure = 2,
  ...
  for(i in 1:max_iter) {
    ...
    for(j in 1:length(clusts_to_use)) {
      ...
      new_centers <- kl_means_cluster(paths, center, path_ids,
    ...
    col_mins <- apply(distm, 2, min)

    col_mins_old <- apply(clust_out$result$distances, 2, min)

    if(sum(col_mins^2) < sum(col_mins_old^2)) {

      message("Improvement in squared distances results in the following: \n",
        "cost: from ", clust_out$result$cost, " to ", new_out$result$cost, "\n",
        "minsum from ", clust_out$result$minsum, " to ", new_out$result$minsum)
...
kl_means_cluster <- function(paths, center, path_ids, center_id, l, prec_l,
  dist_measure, prec_eps, square_dist, norm_dist) {
